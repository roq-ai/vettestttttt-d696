...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['clinic-owner', 'veterinarian', 'veterinary-nurse'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ClinicInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/clinics` : null),
    () => getClinics({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Clinics',
      path: isTenantUser ? `/clinics/view/${data?.[0]?.id}` : '/clinics',
      entity: 'clinic',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Customers', path: '/customers', entity: 'customer', service: AccessServiceEnum.PROJECT, icon: FiUsers },
    { name: 'Events', path: '/events', entity: 'event', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
    { name: 'Notes', path: '/notes', entity: 'note', service: AccessServiceEnum.PROJECT, icon: FiFileText },
    { name: 'Pets', path: '/pets', entity: 'pet', service: AccessServiceEnum.PROJECT, icon: FiFeather },
