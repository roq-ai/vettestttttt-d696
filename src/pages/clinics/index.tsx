...
import { getClinics, deleteClinicById } from 'apiSdk/clinics';
import { ClinicInterface } from 'interfaces/clinic';

type ColumnType = ColumnDef<ClinicInterface, unknown>;

interface ClinicListPageProps {
...
export function ClinicListPage(props: ClinicListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ClinicInterface>>(
    () => `/clinics?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ClinicInterface) => {
    if (hasAccess('clinic', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/clinics/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/clinics/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('clinic', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/clinics/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('clinic', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
