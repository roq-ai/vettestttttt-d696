...
type ColumnType = ColumnDef<PetInterface, unknown>;
...
export function PetListPage(props: PetListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getPets({
        relations: ['customer', 'event.count', 'note.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'species.contains', 'breed.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PetInterface>>(
    () => `/pets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PetInterface) => {
    if (hasAccess('pet', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/pets/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'name', header: 'Name', accessorKey: 'name' },
    { id: 'species', header: 'Species', accessorKey: 'species' },
    { id: 'breed', header: 'Breed', accessorKey: 'breed' },
    { id: 'age', header: 'age', accessorKey: 'age' },
    { id: 'weight', header: 'weight', accessorKey: 'weight' },
    ...
    hasAccess('event', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Event',
          accessorKey: 'event',
          cell: ({ row: { original: record } }: any) => record?._count?.event,
        }
      : null,
    hasAccess('note', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Note',
          accessorKey: 'note',
          cell: ({ row: { original: record } }: any) => record?._count?.note,
        }
      : null,
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="pet" />
        </Flex>

        {hasAccess('pet', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/pets/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
