...
import { NoteListPage as ShowCaseList } from 'pages/notes';

function DashboardPage() {
  const { session } = useSession();

  const petStat = useSWR<PaginatedInterface<PetInterface>>(
    () => `/pets`,
    () => getPets({ limit: 1, offset: 0 }),
  );

  const eventStat = useSWR<PaginatedInterface<EventInterface>>(
    () => `/events`,
    () => getEvents({ limit: 1, offset: 0 }),
  );

  const noteStat = useSWR<PaginatedInterface<NoteInterface>>(
    () => `/notes`,
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Pets',
      result: petStat?.data?.totalCount,
      isLoading: petStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Events',
      result: eventStat?.data?.totalCount,
      isLoading: eventStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Notes',
      result: noteStat?.data?.totalCount,
      isLoading: noteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`notes`} passHref legacyBehavior>
                  <Button
